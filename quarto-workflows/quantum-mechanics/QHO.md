...
Repeat the exercise for methane, using the values $m(CH_{4}) = 16 g \ mol^{-1}$, $a=2.300 \ L^2 bar \ mol^{-2}$, $b = 0.04301 \ L  \ mol^{-1}$, $\Delta_c H_m^{\ominus} (CH_4) = -891 \ kJ \ mol^{-1}$. Comment on the difference between the two gases.
